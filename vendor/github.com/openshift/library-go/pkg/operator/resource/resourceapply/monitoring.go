...
func ApplyServiceMonitor(ctx context.Context, client dynamic.Interface, recorder events.Recorder, required *unstructured.Unstructured, deleteConditionals ...ConditionalFunction) (*unstructured.Unstructured, bool, error) {
	shouldDelete := false
	...
	if shouldDelete {
		err := client.Resource(serviceMonitorGVR).Namespace(namespace).Delete(ctx, existing.GetName(), metav1.DeleteOptions{})
		if err != nil {
			return nil, false, err
		}
...
func ApplyPrometheusRule(ctx context.Context, client dynamic.Interface, recorder events.Recorder, required *unstructured.Unstructured, deleteConditionals ...ConditionalFunction) (*unstructured.Unstructured, bool, error) {
	shouldDelete := false
	// If any of the delete conditionals is true, we should delete the resource
	for _, deleteConditional := range deleteConditionals {
		if deleteConditional() {
	...
	if shouldDelete {
		err := client.Resource(prometheusRuleGVR).Namespace(namespace).Delete(ctx, existing.GetName(), metav1.DeleteOptions{})
		if err != nil {
			return nil, false, err
		}
		reportDeleteEvent(recorder, required, err)
