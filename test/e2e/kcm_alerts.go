...
var _ = g.Describe("kube-controller-manager alert workloads", func() {
	...
	g.It("[OTP][OCP-73887] validate for alert KubeDeploymentReplicasMismatch", func() {
		oc := testlib.NewOC()
		g.DeferCleanup(func() {
			_ = oc.WithoutNamespace().Run("delete").Args("deployment", "replicas-mismatch", "-n", "default", "--ignore-not-found").Execute()
		})

		deployFile := filepath.Join(testlib.FixturePath("workloads"), "deployment-73887.yaml")
		err := oc.WithoutNamespace().Run("create").Args("-f", deployFile, "-n", "default").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		token := testlib.GetSAToken(oc)
		testlib.CheckMetric(oc, testlib.PrometheusURL+` --data-urlencode 'query=ALERTS{alertname="KubeDeploymentReplicasMismatch"}'`, token, "replicas-mismatch", 1200)
	})
	...
	g.It("[OTP][OCP-73874] validate for alert KubeControllerManagerDown [Serial][Disruptive]", func() {
		...
		token := testlib.GetSAToken(oc)
		testlib.CheckMetric(oc, testlib.PrometheusURL+` --data-urlencode 'query=ALERTS{alertname="KubeControllerManagerDown"}'`, token, "openshift-kube-controller-manager", 1200)
	})
})
